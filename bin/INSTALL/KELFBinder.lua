...
function Greeting()
  ...
  while CONTINUE do
    ...
    if IS_NOT_PUBLIC_READY then
      Font.ftPrintMultiLineAligned(LSANS, X_MID, 20, 20, 630, 16, "THIS IS NOT A PUBLIC-READY VERSION!\n"..BETANUM, Color.new(200, 200, 0, Q))
    end
...
function MainMenu()
  ...
  while true do
    ...
    Graphics.drawScaleImage(BG, 0.0, 0.0, SCR_X, SCR_Y)
    ORBMAN(0x80)
    Font.ftPrintMultiLineAligned(LSANS, X_MID, 40, 20, 630, 32, LNG_MM1, Color.new(220, 220, 220, 0x90 - A))
    if T == 1 then
...
function Installmodepicker()
  ...
  while true do
    Screen.clear()
    Graphics.drawScaleImage(BG, 0.0, 0.0, SCR_X, SCR_Y)
    Font.ftPrintMultiLineAligned(LSANS, X_MID, 40, 20, 630, 32, LNG_MM2, Color.new(220, 220, 220, 0x80 - A))
    ORBMAN(0x80)
    if T == 1 then
...
function DVDPlayerRegionPicker()
  ...
  while true do
    ...
    Graphics.drawScaleImage(BG, 0.0, 0.0, SCR_X, SCR_Y)
    ORBMAN(0x80)
    Font.ftPrintMultiLineAligned(LSANS, X_MID, 40, 20, 630, 32, LNG_PICK_DVDPLAYER_REG, Color.new(220, 220, 220, 0x80 - A))
    ...
    Font.ftPrintMultiLineAligned(LSANS, X_MID, 350, 20, 600, 32, PROMTPS[T], Color.new(128, 128, 128, 0x80 - A))
    Promptkeys(1, LNG_CT0, 1, LNG_CT1, 0, 0, A)
    if A > 0 then A = A - 1 end
    Screen.flip()
    local pad = Pads.get()
...
function DVDPlayerINST(port, slot, target_region)
  ...
  if doesFileExist(DVDPLAYERUPDATE) then
    ...
    Screen.clear()
    Graphics.drawScaleImage(BG, 0.0, 0.0, SCR_X, SCR_Y)
    Font.ftPrintMultiLineAligned(LSANS, X_MID, 20, 20, 600, 64, string.format(LNG_INSTPMPT, TARGET_KELF))
    Font.ftPrintMultiLineAligned(LSANS, X_MID, 100, 20, 600, 64, LNG_PLS_WAIT)
    Screen.flip()
...
function MemcardPickup()
  ...
  while true do
    ...
    Graphics.drawScaleImage(BG, 0.0, 0.0, SCR_X, SCR_Y)
    ORBMAN(0x80)
    Font.ftPrintMultiLineAligned(LSANS, X_MID, 40, 20, 630, 32, LNG_MEMCARD0, Color.new(0x80, 0x80, 0x80, 0x80 - A))
    Font.ftPrintMultiLineAligned(LSANS, 160,  270, 20, 630, 32, string.format(LNG_MEMCARD1, 1), Color.new(0x80, 0x80, 0x80, 0x80 - A))
    if mcinfo0.type == 2 then
      if mcinfo0.format == 1 then
        Font.ftPrintMultiLineAligned(LSANS, 160, 290, 20, 630, 32, string.format(LNG_MEMCARD2, mcinfo0.freemem), Color.new(0x80, 0x80, 0x80, 0x80 - A))
      else
        Font.ftPrintMultiLineAligned(LSANS, 160, 290, 20, 630, 32, LNG_UNFORMATTED_CARD, Color.new(0x80, 0, 0, 0x80-A))
      end
    elseif mcinfo0.type ~= 0 then
      Font.ftPrintMultiLineAligned(LSANS, 160, 290, 20, 630, 32, LNG_INCOMPATIBLE_CARD, Color.new(0x80, 0x80, 0x80, 0x80 - A))
    end
    ...
    Font.ftPrintMultiLineAligned(LSANS, MC2_CNTR, 270, 20, 630, 32, string.format(LNG_MEMCARD1, 2), Color.new(0x80, 0x80, 0x80, 0x80 - A))
    if mcinfo1.type == 2 then
      if mcinfo1.format == 1 then
        Font.ftPrintMultiLineAligned(LSANS, MC2_CNTR, 290, 20, 630, 32, string.format(LNG_MEMCARD2, mcinfo1.freemem), Color.new(0x80, 0x80, 0x80, 0x80 - A))
      else
        Font.ftPrintMultiLineAligned(LSANS, MC2_CNTR, 290, 20, 630, 32, LNG_UNFORMATTED_CARD, Color.new(0x80, 0, 0, 0x80-A))
      end
    elseif mcinfo1.type ~= 0 then
      Font.ftPrintMultiLineAligned(LSANS, MC2_CNTR, 290, 20, 630, 32, LNG_INCOMPATIBLE_CARD, Color.new(0x80, 0x80, 0x80, 0x80 - A))
    end
    if T == 1 then
...
function AdvancedINSTprompt()
  ...
  while true do
    ...
    Font.ftPrintMultiLineAligned(LSANS, X_MID, 40, 20, 630, 32, LNG_IMPMP2, Color.new(220, 220, 220, 0x80 - A))
    if T == 1 then
...
function Secrerr(RET)
  ...
  while true do
    ...
    if Q < 0x20 then
      ...
      if RET ~= 1 then
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 40, 20, 630, 64, string.format(LNG_INSTERR, RET), Color.new(0x80, 0x80, 0x80, 0x80 - A))
      else
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 40, 20, 630, 64, LNG_INSTPMPT1, Color.new(0x80, 0x80, 0x80, 0x80 - A))
      end
      if RET == (-5) then
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 60, 20, 630, 64, LNG_EIO, Color.new(0x80, 0x80, 0x80, 0x80 - A))
      elseif RET == (-22) then
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 60, 20, 630, 64, LNG_SECRMANERR, Color.new(0x80, 0x80, 0x80, 0x80 - A))
      elseif RET == (-12) then
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 60, 20, 630, 64, LNG_ENOMEM, Color.new(0x80, 0x80, 0x80, 0x80 - A))
      elseif RET == (-201) or RET == (-203) then
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 60, 20, 630, 64, LNG_SOURCE_KELF_GONE, Color.new(0x80, 0x80, 0x80, 0x80 - A))
        if RET == (-203) then Font.ftPrintMultiLineAligned(LSANS, X_MID, 85, 20, 630, 64, LNG_WARN_DVDPLAYER_PROPIETARY_SOFTWARE , Color.new(0x80, 0x80, 0x80, 0x80 - A)) end
      elseif RET == (-202) then
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 80 , 20, 630, 64, LNG_MBR_KELF_SIZE_OUT_OF_BOUNDS , Color.new(0x80, 0x80, 0x80, 0x80 - A))
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 120, 20, 630, 64, LNG_MBR_KELF_SIZE_OUT_OF_BOUNDS2, Color.new(0x80, 0x80, 0x80, 0x80 - A))
...
function Report(RET, IS_GOOD, IS_A_QUESTION)
  ...
  while true do
    ...
    if Q < 0x20 then
      ...
      if RET == 101 then
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 60, 20, 630, 64, LNG_HDDBOOT_ALREADY_ENABLED, Color.new(0x80, 0x80, 0x80, 0x80 - A))
      elseif RET == 100 then
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 60, 20, 630, 64, LNG_HDDBOOT_ENABLED, Color.new(0x80, 0x80, 0x80, 0x80 - A))
      elseif RET == 200 then
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 60,  20, 630, 64, LNG_HDDFORMAT_CONFIRM, Color.new(0x80, 0x80, 0x80, 0x80 - A))
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 100, 20, 630, 64, LNG_HDDFORMAT_CONFIRM2, Color.new(0x80, 0x80, 0x80, 0x80 - A))
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 140, 20, 630, 64, LNG_CONTINUE.."?", Color.new(0x80, 0x80, 0x80, 0x80 - A))
      elseif RET == 300 then
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 60,  20, 630, 64, LNG_HDD_SMART_STATUS_FAILS_WARNING, Color.new(0x80, 0x80, 0, 0x80 - A))
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 140, 20, 630, 64, LNG_HDD_RECOMMEND_HDD_REPLACE, Color.new(0x80, 0x80, 0x80, 0x80 - A))
      elseif RET == 301 then
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 60,  20, 630, 64, LNG_HDD_SECTOR_ERROR_WARNING, Color.new(0x80, 0x80, 0, 0x80 - A))
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 140, 20, 630, 64, LNG_HDD_RECOMMEND_HDD_REPLACE, Color.new(0x80, 0x80, 0x80, 0x80 - A))
      elseif RET == 302 then
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 60,  20, 630, 64, LNG_HDD_CORRUPTED_PART_WARNING, Color.new(0x80, 0x80, 0, 0x80 - A))
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 140, 20, 630, 64, LNG_HDD_RECOMMEND_FORMAT_OR_FSCK, Color.new(0x80, 0x80, 0x80, 0x80 - A))
      elseif RET == 400 then
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 60,  20, 630, 64, LNG_DEX_MACHINE_WARNING, Color.new(0x80, 0x80, 0, 0x80 - A))
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 140, 20, 630, 64, LNG_DEX_MACHINE_WARNING_DESC, Color.new(0x80, 0x80, 0x80, 0x80 - A))
      end
      if Pads.check(pad, PAD_CROSS) and A == 0 then
        ret = true
        QIN = -1
...
function MagicGateTest(port, slot)
  ...
  while A > 0 do
    Screen.clear()
    Graphics.drawScaleImage(BG, 0.0, 0.0, SCR_X, SCR_Y, Color.new(0x80, 0x80, 0x80, A))
    Font.ftPrintMultiLineAligned(LSANS, X_MID, 40, 20, 630, 64, LNG_PLS_WAIT, Color.new(0x80, 0x80, 0x80, 0x80 - A))
    A = A - 1
    Screen.flip()
  end
  local RET
  ...
  while true do
    ...
    if Q < 0x20 then
      ...
      Promptkeys(1, LNG_CONTINUE, 0, 0, 0, 0, A)
      if RET ~= 1 then
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 40, 20, 630, 64, string.format(LNG_TESTTERR, RET), Color.new(0x80, 0x80, 0x80, 0x80 - A))
      else
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 40, 20, 630, 64, LNG_TESTSUCC, Color.new(0x80, 0x80, 0x80, 0x80 - A))
      ...
      elseif RET == (-201) then
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 60, 20, 630, 64, LNG_SOURCE_KELF_GONE, Color.new(0x80, 0x80, 0x80, 0x80 - A))
      elseif RET ~= 1 then -- only write unknown error if retcode is not a success
        Font.ftPrintMultiLineAligned(LSANS, X_MID, 60, 20, 630, 64, LNG_EUNKNOWN, Color.new(0x80, 0, 0, 0x80 - A))
      end
...
function WarnOfShittyFMCBInst()
  ...
  while true do
    ...
    Graphics.drawScaleImage(BGERR, 0.0, 0.0, SCR_X, SCR_Y, Color.new(0x80, 0x80, 0x80, 0x80 - Q))
    ORBMANex(REDCURSOR, 0x80 - Q - 1, 180, 180, 80 + Q)
    Font.ftPrintMultiLineAligned(LSANS, X_MID,  40, 20, 630, 64, LNG_WARNING, Color.new(0x80, 0x80, 0x80, 0x80 - Q))
    Font.ftPrintMultiLineAligned(LSANS, X_MID,  80, 20, 630, 64, LNG_FMCBINST_CRAP0, Color.new(0x80, 0x80, 0x80, 0x80 - Q))
    Font.ftPrintMultiLineAligned(LSANS, X_MID, 120, 20, 630, 64, LNG_FMCBINST_CRAP1, Color.new(0x80, 0x80, 0x80, 0x80 - Q))
    Font.ftPrintMultiLineAligned(LSANS, X_MID, 190, 20, 630, 64, LNG_FMCBINST_CRAP2, Color.new(0x80, 0x80, A, 0x80 - Q))
...
function InsufficientSpace(NEEDED, AVAILABLE, targetdev)
  ...
  while true do
    ...
    Graphics.drawScaleImage(BGERR, 0.0, 0.0, SCR_X, SCR_Y, Color.new(0x80, 0x80, 0x80, 0x80 - Q))
    ORBMANex(REDCURSOR, 0x80 - Q - 1, 180, 180, 80 + Q)
    Font.ftPrintMultiLineAligned(LSANS, X_MID,  40, 20, 630, 64, LNG_ERROR, Color.new(0x80, 0x80, 0x80, 0x80 - Q))
    Font.ftPrintMultiLineAligned(LSANS, X_MID,  80, 20, 630, 64, string.format(LNG_NOT_ENOUGH_SPACE0, targetdev), Color.new(0x80, 0x80, 0x80, 0x80 - Q))
    Font.ftPrintMultiLineAligned(LSANS, X_MID, 120, 20, 630, 64, string.format(LNG_NOT_ENOUGH_SPACE1, NEEDED / 1024, AVAILABLE / 1024),
...
function Ask2WipeSysUpdateDirs(NEEDS_JPN, NEEDS_USA, NEEDS_EUR, NEEDS_CHN, NEEDS_CURRENT, port)
  ...
  while true do
    ...
    if Q < 0x20 then
      ...
      Font.ftPrint(LSANS, 50, 040, 0, 630, 64, LNG_WARNING, Color.new(0x80, 0x80, 0x80, 0x80 - A))
...
function WarnIncompatibleMachine()
  ...
  while true do
    ...
    Font.ftPrintMultiLineAligned(LSANS, X_MID, 40, 20, 630, 64, LNG_COMPAT0.."\n"..LNG_COMPAT1, Color.new(0x80, 0x80, 0x80, 0x80 - Q))
    if Pads.check(pad, PAD_CROSS) then
      QIN = -1
      Q = 1
    end
...
function PerformExpertINST(port, slot, UPDT)
  ...
  Screen.clear()
  Graphics.drawScaleImage(BG, 0.0, 0.0, SCR_X, SCR_Y)
  Font.ftPrintMultiLineAligned(LSANS, X_MID, 40, 20, 600, 64, LNG_CALCULATING)
  Screen.flip()
...
function ReportProgress(prog, total)
  Screen.clear()
  Graphics.drawScaleImage(BG, 0.0, 0.0, SCR_X, SCR_Y)
  Font.ftPrintMultiLineAligned(LSANS, X_MID, 40, 20, 600, 64, LNG_INSTALLING)
...
function Ask2quit()
  ...
  while true do
    ...
    Screen.clear()
    Graphics.drawScaleImage(BG, 0.0, 0.0, SCR_X, SCR_Y)
    Font.ftPrintMultiLineAligned(LSANS, X_MID, 40, 20, 630, 16, LNG_WANNAQUIT)
...
function SystemInfo()
  ...
  while true do
    ...
    Graphics.drawScaleImage(BG, 0.0, 0.0, SCR_X, SCR_Y)
    ORBMAN(0x80)
    Font.ftPrintMultiLineAligned(LSANS, X_MID, 20, 20, 630, 32, LNG_SYSTEMINFO, Color.new(220, 220, 220, 0x80 - A))
